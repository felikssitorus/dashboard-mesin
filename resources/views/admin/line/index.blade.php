...
@section('breadcrumb')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
        ...
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item text-muted">Permission Line Manage</li>
        <!--end::Item-->
    </ul>
    <!--end::Breadcrumb-->
@endsection
...
@section('scripts')
    ...
    <script>
        const _URL = "{{ route('admin.permissionLine.getDataTable') }}";
        ...
        $(document).ready(function () {
            ...
            let DT = $("#dt_permission_line").DataTable({
                order: [[1, 'asc']],
                processing: true,
                serverSide: true,
                ajax: {
    ...
    <script>
        function deleteRuang(id) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: "{{ route('admin.permissionLine.destroy') }}",
                        type: "POST",
                        data: {
                            id: id,
                            _token: "{{ csrf_token() }}",
                        ...
                        success: function (response) {
                            $("#dt_permission_line").DataTable().ajax.reload(null, false);
